  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const charactersByRarity = {
        ...
        legendary: setCharacters.filter(c => c.rarity === 'legendary')
      };
      ...
      for (let i = 0; i < cardCount; i++) {
        ...
        } else if (random < rarityDistribution.common + rarityDistribution.uncommon) {
          selectedRarity = 'uncommon';
        } else if (random < rarityDistribution.common + rarityDistribution.uncommon + rarityDistribution.rare) {
          selectedRarity = 'rare';
        } else {
        ...
        const userCard = await tx.userCard.create({
          data: {
            ...
            condition,
            isHolo: selectedRarity === 'rare' || selectedRarity === 'legendary' ? Math.random() < 0.3 : false,
