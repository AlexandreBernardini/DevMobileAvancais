...
const createGame = async (req, res, next) => {
  try {
    const { player2Id, player1DeckId, player2DeckId, gameMode, format, tournamentId } = req.body;
    ...
    if (player1Deck.format !== player2Deck.format || player1Deck.format !== format) {
      return res.status(400).json({
        success: false,
...
const getGameById = async (req, res, next) => {
  try {
    ...
    if (!isParticipant && !isSpectatorAllowed && req.user.role !== 'admin') {
